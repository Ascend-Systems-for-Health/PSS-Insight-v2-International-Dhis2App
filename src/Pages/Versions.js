...
export default function Versions({ user }) {
  ...
  const columns = [
    ...
    },
    {
      title: "DATE CREATED",
      key: "createdAt",
      render: (_, row) => row.createdAt && format(new Date(row.createdAt), "dd/MM/yyyy"),
    },
    {
      title: "CREATED BY",
      key: "createdBy",
      ...
      render(row) {
        return (
          <div className={classes.actions}>
            ...
            {row?.status !== "PUBLISHED" && row.createdBy === user?.me?.username ? (
              <>
                <Link to={`/templates/versions/edit/${row.id}`} className={classes.actionLink}>
